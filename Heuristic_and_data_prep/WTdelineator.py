...
def findWaveLimits(w, beg, thresh, on, win):
    ...
    while abs(searchPos - beg) < win and searchPos >= 1 and searchPos <= w.shape[
        0] - 2:  # Loop only within the search limits
        # If the conditions for wave limit are met, return the position
        if (abs(w[searchPos]) < thresh) or (
                abs(w[searchPos]) > abs(w[searchPos + 1]) and abs(w[searchPos]) > abs(w[searchPos - 1])):
            return [searchPos]
...
def Rdetection(w, fs):
    ...
    for i in range(1, len(n1)):
        ...
        if (n1[i] - n1[i - 1] < win) and (
                (w[0][n1[i]] > 0 and w[0][n1[i - 1]] < 0) or (w[0][n1[i]] < 0 and w[0][n1[i - 1]] > 0)):
